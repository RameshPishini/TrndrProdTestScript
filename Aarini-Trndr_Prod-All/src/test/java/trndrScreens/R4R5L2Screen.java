...
public class R4R5L2Screen {
	...
	@FindBy(how=How.XPATH,using="//*[@id=\"0\"]")
	static	WebElement L2card2src1;
	
	@FindBy(how=How.XPATH,using="//*[@id=\"root\"]/div/div/div[1]/div[1]")
	static	WebElement L2card2tar1;
	
	
	
	
	@FindBy(how=How.XPATH,using="//*[@id=\"root\"]/div/div/div[1]/div[3]/a")
  ...
  public static WebElement level2(WebDriver driver) throws InterruptedException {
	...
	L2popupbtn.click();
			...
			act.clickAndHold(L2card2src1).build().perform();		
			act.moveToElement(L2card2tar1).build().perform();			
			act.release(L2card2tar1).build().perform();		
			System.out.println("released l2 src-tar card");
