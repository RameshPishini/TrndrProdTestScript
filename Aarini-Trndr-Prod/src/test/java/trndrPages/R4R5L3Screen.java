...
public class R4R5L3Screen {
	...
	@FindBy(how=How.XPATH,using="//*[@id=\"cards\"]/div/div/div/div[2]")
	static	WebElement L3card2src1;
	
	
	@FindBy(how=How.XPATH,using="//*[@id=\"root\"]/div/div/div[1]/span/div[1]/div[2]")
	static	WebElement L3card2tar1;
	...
	public static WebElement level3(WebDriver driver) {
		...
		l3popupbtn1.click();
		   ...
		   wait.until(ExpectedConditions.elementToBeClickable(L3card1src1));
			...
			wait.until(ExpectedConditions.elementToBeClickable(L3card2src1));
			 act.clickAndHold(L3card2src1).build().perform();		
				act.moveToElement(L3card2tar1).build().perform();			
				act.release(L3card2tar1).build().perform();	
